...
Make sure that all grammar rule definitions are done before 'performSelfAnalysis' is called.`);
 ...
 ${defErrorsMsgs.join("\n-------------------------------\n")}`);
  ...
  var BlockComment = createToken({ name: "BlockComment", pattern: /\/-/ });
  ...
  var OpenMultiLineComment = createToken({
    name: "OpenMultiLineComment",
    pattern: /\/\*/,
    push_mode: "multilineComment"
  });
  var MultiLineCommentContent = createToken({
    name: "MultiLineCommentContent",
    pattern: /([^/*]+|\*|\/)/,
    line_breaks: true
  });
  var CloseMultiLineComment = createToken({
  ...
  var Boolean = createToken({ name: "Boolean", pattern: /#true|#false/ });
  var Null = createToken({ name: "Null", pattern: /#null/ });
  var FloatKeyword = createToken({ name: "FloatKeyword", pattern: /#inf|#-inf|#nan/ });
  var RawString = createToken({
    name: "RawString",
    // eslint-disable-next-line no-control-regex
    pattern: /(#+)"([^\x00-\x08\x0A-\x1F\x7F\x85\u200E\u200F\u2028-\u202E\u2066-\u2069\uD800-\uDFFF\uFEFF]|[\uD800-\uDBFF][\uDC00-\uDFFF])*?"\1/
  });
  var MultiLineRawString = createToken({
    name: "MultiLineRawString",
    // eslint-disable-next-line no-control-regex
    pattern: /(#+)"""([^\x00-\x08\x0E-\x1F\x7F\u200E\u200F\u202A-\u202E\u2066-\u2069\uD800-\uDFFF\uFEFF]|[\uD800-\uDBFF][\uDC00-\uDFFF])*?"""\1/,
    line_breaks: true
  });
  var Float = createToken({
    name: "Float",
    pattern: /[+-]?[0-9][0-9_]*(\.[0-9][0-9_]*)?([eE][+-]?[0-9][0-9_]*)?/
  });
  var Integer = createToken({
    name: "Integer",
  ...
  var LeftParenthesis = createToken({ name: "LeftParenthesis", pattern: /\(/ });
  var RightParenthesis = createToken({ name: "RightParenthesis", pattern: /\)/ });
  var EscLine = createToken({ name: "EscLine", pattern: /\\/ });
  var Unknown = createToken({ name: "Unknown", pattern: /[^]/ });
  var OpenQuote = createToken({ name: "OpenQuote", pattern: /"/, push_mode: "string" });
  var Unicode = createToken({
    name: "Unicode",
    // eslint-disable-next-line no-control-regex
    pattern: /([^\\"\x00-\x08\x0A-\x1F\x7F\x85\u200E\u200F\u2028-\u202E\u2066-\u2069\uD800-\uDFFF\uFEFF]|[\uD800-\uDBFF][\uDC00-\uDFFF])+/
  });
  var Escape = createToken({ name: "Escape", pattern: /\\[nrt\\"bfs]/ });
  var UnicodeEscape = createToken({ name: "UnicodeEscape", pattern: /\\u\{([0-9a-fA-F]{1,5}|10[0-9a-fA-F]{4})\}/ });
  var WhiteSpaceEscape = createToken({
    name: "WhiteSpaceEscape",
    // eslint-disable-next-line no-control-regex
    pattern: /\\[\x09\x0A-\x0D\x20\x85\xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000]+/,
    line_breaks: true
  });
  var CloseQuote = createToken({ name: "CloseQuote", pattern: /"/, pop_mode: true });
  var MultiLineOpenQuote = createToken({ name: "MultiLineOpenQuote", pattern: /"""/, push_mode: "multilineString" });
  var MultiLineCloseQuote = createToken({ name: "MultiLineCloseQuote", pattern: /"""/, pop_mode: true });
  var MultiLineSingleQuote = createToken({ name: "MultiLineSingleQuote", pattern: /"/ });
  ...
  var GreaterOrEqualThan = createToken({ name: "GreaterOrEqualThan", pattern: />=/ });
  var LessOrEqualThan = createToken({ name: "LessOrEqualThan", pattern: /<=/ });
  var Or = createToken({ name: "Or", pattern: /\|\|/ });
  var Descendant = createToken({ name: "Descendant", pattern: />>/ });
  var AdjacentSibling = createToken({ name: "AdjacentSibling", pattern: /\+/ });
  var Sibling = createToken({ name: "Sibling", pattern: /\+\+/ });
  var NotEquals = createToken({ name: "NotEquals", pattern: /!=/ });
  var StartsWith = createToken({ name: "StartsWith", pattern: /\^=/ });
  var EndsWith = createToken({ name: "EndsWith", pattern: /\$=/ });
  var Contains = createToken({ name: "Contains", pattern: /\*=/ });
  var Map3 = createToken({ name: "Map", pattern: /=>/ });
  var TopAccessor = createToken({ name: "TopAccessor", pattern: /top\(/ });
  var ValAccessor = createToken({ name: "ValAccessor", pattern: /val\(/ });
  var PropAccessor = createToken({ name: "PropAccessor", pattern: /prop\(/ });
  var Accessor = createToken({ name: "Accessor", pattern: /(name|tag|values|props)\(/ });
  ...
  var bannedIdentifiers = /* @__PURE__ */ new Set([
    "true",
    "false",
    "null",
    "inf",
  ...
  var BaseParser = class extends EmbeddedActionsParser {
    constructor(tokens3) {
      super(tokens3);
      this.RULE("tag", () => {
      ...
      this.RULE("nonStringValue", () => {
        return this.OR([
          ...
          {
            ALT: () => {
              const keyword = this.CONSUME(FloatKeyword).image;
      ...
      this.RULE("multilineString", () => {
        this.CONSUME(MultiLineOpenQuote);
        const lines = [];
        this.AT_LEAST_ONE(() => {
          const newline = this.CONSUME(NewLine);
          const prefix = this.OPTION(() => this.CONSUME(WhiteSpace));
          let line = "";
          this.MANY1(() => {
            this.OR([
              { ALT: () => {
                line += this.CONSUME(MultiLineSingleQuote).image;
              } },
              { ALT: () => {
                line += this.CONSUME(Unicode).image;
              } },
              { ALT: () => {
                line += escapes[this.CONSUME(Escape).image];
              } },
              { ALT: () => {
                line += this.SUBRULE(this.unicodeEscape);
              } },
              { ALT: () => {
                this.CONSUME(WhiteSpaceEscape);
              } },
              { ALT: () => {
                line += this.CONSUME1(WhiteSpace).image;
              } },
              {
                GATE: () => this.LA(1).tokenType === MultiLineCloseQuote,
                ALT: () => {
                }
              }
            ]);
        ...
        this.CONSUME(MultiLineCloseQuote);
        return this.ACTION(() => {
          const end = lines.pop();
          if (end.line !== "") {
      ...
      this.RULE("rawString", () => {
        const previousToken = this.LA(0);
        const token = this.CONSUME(RawString);
      ...
      this.RULE("multilineRawString", () => {
        const token = this.CONSUME(MultiLineRawString);
        const string = token.image;
        ...
        const newlinePattern = new RegExp(NewLine.PATTERN.source, "g");
        ...
        const whitespacePattern = new RegExp("^(" + WhiteSpace.PATTERN.source + ")$");
      ...
      this.RULE("unicodeEscape", () => {
        const prevToken = this.LA(0);
        const escape = this.CONSUME(UnicodeEscape);
...
`;
      output += error.context.ruleStack.map((rule) => "  " + rule).join("\n");
      return output;
    }
  };
  ...
  var nodeEndTokens = /* @__PURE__ */ new Set([
    RightBrace,
    LineComment,
    NewLine,
    SemiColon,
  ...
  var KdlParser = class extends BaseParser {
    constructor() {
      super(tokens);
      this.RULE("document", () => {
        this.OPTION(() => this.CONSUME(BOM));
      ...
      this.RULE("node", () => {
        ...
        if (this.LA(1).tokenType !== RightBrace) {
        ...
        return this.OR([
          {
            ALT: () => {
              ...
              return [void 0, value, tag, hasSpaceAfter];
            }
          },
          {
            ALT: () => {
              ...
              return [void 0, value, void 0, hasSpaceAfter];
            }
          },
          {
            ALT: () => {
              ...
              this.OPTION4(() => {
                ...
                name = value;
                tag = this.OPTION6(() => {
                  const tag2 = this.SUBRULE1(this.tag);
  ...
  function transformLexerError(error, tokens3, text) {
    const endOffset = error.offset + error.length;
    const image = text.slice(error.offset, endOffset);
    const lines = image.split(/\r?\n/g);
    const prevToken = tokens3.find((token) => token.endOffset + 1 === error.offset);
  ...
  function parse(text) {
    const { tokens: tokens3, errors } = lexer.tokenize(text);
    if (errors.length) {
      return {
        output: void 0,
        errors: errors.map((error) => transformLexerError(error, tokens3, text))
      };
    }
    parser.input = tokens3;
  ...
  function validateNode(node) {
    ...
    if (!Object.prototype.hasOwnProperty.call(node, "name") || typeof node.name !== "string") {
      return false;
    }
    if (!Object.prototype.hasOwnProperty.call(node, "values") || !Array.isArray(node.values) || !node.values.every((value) => validateValue(value))) {
      return false;
    }
    if (!Object.prototype.hasOwnProperty.call(node, "properties") || typeof node.properties !== "object" || !Object.values(node.properties).every((value) => validateValue(value))) {
      return false;
    }
    if (!Object.prototype.hasOwnProperty.call(node, "children") || !validateDocument(node.children)) {
      return false;
    }
    if (!Object.prototype.hasOwnProperty.call(node, "tags") || !validateTags(node.tags)) {
      return false;
    }
    return true;
  }
  ...
  var nonPrintableAscii = /^[\x00-\x19\x7F]$/;
  var commonEscapes = {
    "\n": "\\n",
    "\r": "\\r",
    "	": "\\t",
  ...
  var identifierPattern = new RegExp("^(" + Identifier.PATTERN.source + ")$");
  ...
  function formatTag(value, options) {
    return value === void 0 ? "" : "(" + formatIdentifier(value, options) + ")";
  }
  ...
  function formatNode(node, options, indent) {
    let values2 = node.values;
    if (!options.printNullArgs) {
      values2 = values2.filter((value) => value !== null);
    }
    ...
    const parts = [
      ...
      ...values2.map((value, index) => formatValue(value, node.tags.values[index], options)),
  ...
  var KqlParser = class extends BaseParser {
    constructor() {
      super(tokens2);
      this.RULE("query", () => {
      ...
      this.RULE("matcher", () => {
        return this.OR([
          ...
          {
            ALT: () => {
              this.CONSUME1(LeftParenthesis);
              this.CONSUME1(RightParenthesis);
      ...
      this.RULE("accessorMatcher", () => {
        ...
        const matcher = this.OPTION(() => {
          ...
          const matcher2 = this.OPTION1(() => {
            this.CONSUME(WhiteSpace);
            const operator = this.SUBRULE(this.matcherOperator);
            this.CONSUME1(WhiteSpace);
            const matcher3 = { accessor, operator };
            this.OR([
              { ALT: () => {
                matcher3.value = this.SUBRULE(this.value);
              } },
              { ALT: () => {
                matcher3.tag = this.SUBRULE(this.tag);
              } }
            ]);
            return matcher3;
          });
          return matcher2 || { accessor };
        });
      ...
      this.RULE("accessor", () => {
        return this.OR([
          ...
          {
            ALT: () => {
              const type = this.CONSUME(Accessor).image.slice(0, -1);
              this.CONSUME2(RightParenthesis);
              return { type };
            }
          },
          {
      ...
      this.RULE("selectionOperator", () => {
        return this.OR([
          ...
          { ALT: () => this.CONSUME(AdjacentSibling) },
          { ALT: () => this.CONSUME(Sibling) }
        ]).image;
      });
  ...
  var lexer2 = new Lexer(tokens2);
  var parser2 = new KqlParser();
  function parse3(text) {
    parser2.input = lexer2.tokenize(text).tokens;
    const output = parser2.query();
    return {
      output,
      errors: parser2.errors
    };
  }
  ...
  function applyQuery(query2, doc) {
    const nodes = unique(query2.alternatives.flatMap((selector) => applySelector(selector, doc)));
    if (query2.mapping) {
      return applyMapping(query2.mapping, nodes);
    } else {
      return nodes;
    }
  }
  function query(doc, queryString) {
    ...
    const { output, errors } = parse3(queryString);
